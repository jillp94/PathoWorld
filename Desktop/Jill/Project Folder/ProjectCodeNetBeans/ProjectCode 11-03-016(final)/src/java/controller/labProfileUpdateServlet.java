 ...
 * @author Sony
 */
@WebServlet(name = "labProfileUpdateServlet", urlPatterns = {"/labProfileUpdateServlet"})
@MultipartConfig(fileSizeThreshold=1024*1024*10,    // 10 MB 
                 maxFileSize=1024*1024*50,          // 50 MB
                 maxRequestSize=1024*1024*100)      // 100 MB
public class labProfileUpdateServlet extends HttpServlet {
...
String lhrs=lhr1+" To "+lhr2;
           ...
           System.out.println("values in update servlet are: --"+i+"...."+values[i]);
           }
            model.database m1 = new model.database();
            if (m1.updateLabProfile(values, m1.id)) {
               // out.println("successfully updated");
                request.getRequestDispatcher("UpdateLabProfile.jsp?er=1").forward(request, response);
            } else {
